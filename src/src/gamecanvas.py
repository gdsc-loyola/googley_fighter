...
class GameCanvas:
        ...
        while menu_running:
            ...
            if frame_timer >= frame_delay:
                for char in frame_index:
                    ...
                    if frames:  # only if frames loaded
            ...
            for char, pos in positions.items():
                ...
                if frames and len(frames) > 0:
                    frame = frames[frame_index[char]]   # ✅ use existing index
                else:
                    if frame_index[char] != -1:
    ...
    def character_select(self):
        ...
        # Load static preview images (idle.png) scaled to 64x64
        previews = {
            "Googley": pygame.transform.scale(
                pygame.image.load("assets/images/googley/idle.png").convert_alpha(),
                (64, 64)
            ),
            "Steve": pygame.transform.scale(
                pygame.image.load("assets/images/steve/idle.png").convert_alpha(),
                (64, 64)
            ),
            "Alex": pygame.transform.scale(
                pygame.image.load("assets/images/alex/idle.png").convert_alpha(),
                (64, 64)
            ),
        }
        ...
        while choosing:
            ...
            for i, box in enumerate(boxes):
                ...
                # Draw static preview image
                char = characters[i]
                preview_img = previews[char]
                self.screen.blit(
                    preview_img,
